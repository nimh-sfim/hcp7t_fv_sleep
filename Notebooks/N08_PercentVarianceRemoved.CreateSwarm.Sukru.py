...
# ==========================================================
i=0
os.system('echo "#swarm -f ./N08_PercentVarianceRemoved.SWARM.Sukru.sh -g 4 -t 4 -b 30 --time=00:05:00 --partition quick,norm --logdir ./N08_PercentVarianceRemoved.logs" > ./N08_PercentVarianceRemoved.SWARM.Sukru.sh')
for sbj_run in Manuscript_Runs:
    sbj,run  = sbj_run.split('_',1)
    suffix = 'PVR.BASIC.nii.gz'
    path = osp.join(DATA_DIR,sbj,run,'{run}_{suffix}'.format(run=run, suffix=suffix))
    if not osp.exists(path):
        print ('++ WARNING: Primary output missing [%s]' % path)
        os.system('echo "export SBJ={sbj} RUN={run}; sh ./N08_PercentVarianceRemoved.sh" >> ./N08_PercentVarianceRemoved.SWARM.Sukru.sh'.format(sbj=sbj, run=run, ddir=DATA_DIR))
        i = i +1
print(i)
