...
if not osp.exists('./N13_FCMatrix_Schaefer200.logs'):
    print('++ INFO: Creating logging dir')
    os.mkdir('./N13_FCMatrix_Schaefer200.logs')

# Create Swarm file for extracting representative power
# ==========================================================
os.system('echo "#swarm -f ./N13_FCMatrix_Schaefer200.SWARM.sh -g 16 -t 16 --partition quick,norm --module afni --logdir ./N13_FCMatrix_Schaefer200.logs" > ./N13_FCMatrix_Schaefer200.SWARM.sh')
for sbj_run in Manuscript_Runs:
    sbj,run  = sbj_run.split('_',1)
    os.system('echo "export SBJ={sbj} RUN={run}; sh ./N13_FCMatrix_Schaefer200.sh" >> ./N13_FCMatrix_Schaefer200.SWARM.sh'.format(sbj=sbj, run=run))
...
# # rm ./N13_FCMatrix_Schaefer200.logs/*
# swarm -f ./N13_FCMatrix_Schaefer200.SWARM.sh -g 16 -t 16 --partition quick,norm --module afni --logdir ./N13_FCMatrix_Schaefer200.logs
# ```
...
for sbj_run in Manuscript_Runs:
    ...
    for suffix in ['BASIC', 'Behzadi_COMPCOR', 'AFNI_COMPCOR', 'AFNI_COMPCORp']:
        ...
        if not osp.exists(file):
            print('++ WARNING: [%s] is missing.' % file)
...
# +
ET_Groups  = {'Awake':Awake_Runs,'Drowsy':Drowsy_Runs}
GS_Groups  = {'Drowsy':np.loadtxt(osp.join(Resources_Dir,'Run_List_GS_Top100.txt'),dtype=str),
              'Awake':np.loadtxt(osp.join(Resources_Dir,'Run_List_GS_Bot100.txt'),dtype=str)}
PSD_Groups = {'Drowsy':np.loadtxt(osp.join(Resources_Dir,'Run_List_PSD_Top100.txt'),dtype=str),
              'Awake':np.loadtxt(osp.join(Resources_Dir,'Run_List_PSD_Bot100.txt'),dtype=str)}
...
print('++ ET-Based Grouping          : Awake = %d, Drowsy = %d' %(len(ET_Groups['Awake']),len(ET_Groups['Drowsy'])))
print('++ GS-Based Grouping          : Awake = %d, Drowsy = %d' %(len(GS_Groups['Awake']),len(GS_Groups['Drowsy'])))
print('++ PSD-Based Grouping         : Awake = %d, Drowsy = %d' %(len(PSD_Groups['Awake']),len(PSD_Groups['Drowsy'])))
# -
...
Mean_CCs     = {}
for suffix in ['BASIC', 'Behzadi_COMPCOR', 'AFNI_COMPCOR', 'AFNI_COMPCORp']:
    for group_criteria,groups in zip(['ET','PSD','GS'],
                                     [ET_Groups,PSD_Groups,GS_Groups]):
        # Compute average FC matrix per run type
        Mean_CCs[(suffix,group_criteria,'Awake', 'Z')]  = pd.DataFrame(Z_matrix[suffix].loc[groups['Awake'] ,:,:].mean(axis=0).values)
        Mean_CCs[(suffix,group_criteria,'Drowsy','Z')]  = pd.DataFrame(Z_matrix[suffix].loc[groups['Drowsy'],:,:].mean(axis=0).values)
        Mean_CCs[(suffix,group_criteria,'Awake','R')]   = Mean_CCs[(suffix,group_criteria,'Awake','Z')].apply(np.tanh)
        Mean_CCs[(suffix,group_criteria,'Drowsy','R')]  = Mean_CCs[(suffix,group_criteria,'Drowsy','Z')].apply(np.tanh)
        Mean_CCs[(suffix,group_criteria,'Awake-Drowsy','Z')]             = Mean_CCs[(suffix,group_criteria,'Awake','Z')] - Mean_CCs[(suffix,group_criteria,'Drowsy','Z')]
        Mean_CCs[(suffix,group_criteria,'Awake-Drowsy','R')]             = Mean_CCs[(suffix,group_criteria,'Awake-Drowsy','Z')].apply(np.tanh)
        Mean_CCs[(suffix,group_criteria,'100*(Awake-Drowsy)/Awake','R')] = 100 * Mean_CCs[(suffix,group_criteria,'Awake-Drowsy','R')] / Mean_CCs[(suffix,group_criteria,'Awake','Z')].apply(np.tanh)

# + [markdown] tags=[]
# ### Generate Plotting Infrastructure

# +
preprocessing_labels={'BASIC':'mPP + Motion + Drifts + Bandpass',
                      'Behzadi_COMPCOR':'HCP + Motion + Drifts + Bandpass + Behzadi CompCorr (5PCAs from vents and WM)',
                      'AFNI_COMPCOR':'HCP + Motion + Drifts + Bandpass + AFNI CompCorr (3PCAs from lat vents)',
                      'AFNI_COMPCORp':'HCP + Motion + Drifts + Bandpass + AFNI CompCorr + Static 4th Vent'}
select_preprocessing = pn.widgets.Select(name='Pre-processing', options=['BASIC', 'Behzadi_COMPCOR', 'AFNI_COMPCOR', 'AFNI_COMPCORp'], 
                                         value='BASIC')
...
@pn.depends(select_preprocessing, select_grouping)
def plot_drowsyFC(suffix,grouping):
    drowsy_matrix = hv.Image(Mean_CCs[(suffix,grouping,'Drowsy','R')].values[::-1], bounds=(0,0,Nrois,Nrois)).opts(clim=(-0.75,0.75), cmap='RdBu_r', aspect='square', frame_width=300, 
...
@pn.depends(select_preprocessing, select_grouping)
def plot_awakeFC(suffix,grouping):
    awake_matrix = hv.Image(Mean_CCs[(suffix,grouping,'Awake','R')].values[::-1], bounds=(0,0,Nrois,Nrois)).opts(clim=(-0.75,0.75), cmap='RdBu_r', aspect='square', frame_width=300, 
                                                                                              title='', fontsize={'ticks':12, 'title':16}, 
                                                                                              xlabel='', ylabel='',
                                                                                              colorbar=True, xaxis=None, yaxis=None).opts(toolbar=None)
    net_segments_y = hv.Segments(net_data, 
...
@pn.depends(select_preprocessing, select_grouping)
def plot_diffFC(suffix,grouping):
    diff_matrix   = hv.Image(Mean_CCs[(suffix,grouping,'Awake-Drowsy','R')].values[::-1], bounds=(0,0,Nrois,Nrois)).opts(clim=(-0.20,0.20), cmap='PiYG_r', aspect='square', frame_width=300, 
...
@pn.depends(select_preprocessing, select_grouping)
def plot_staticFC_scan_level(suffix,grouping):
    awake_matrix = hv.Image(Mean_CCs[(suffix,grouping,'Awake','R')].values[::-1], bounds=(0,0,200,200)).opts(clim=(-0.75,0.75), cmap='RdBu_r', aspect='square', frame_width=300, 
                                                                                              title='Average (Awake)', fontsize={'ticks':12, 'title':18}, 
                                                                                              yticks=network_labels_tuples, xticks=[(50,'Left'),(100,'|'),(150,'Right')], colorbar=True)
     
    drowsy_matrix = hv.Image(Mean_CCs[(suffix,grouping,'Drowsy','R')].values[::-1], bounds=(0,0,200,200)).opts(clim=(-0.75,0.75), cmap='RdBu_r', aspect='square', frame_width=300, 
                                                                                              title='Average (Drowsy)', fontsize={'ticks':12, 'title':18}, 
                                                                                              yticks=network_labels_tuples, xticks=[(50,'Left'),(100,'|'),(150,'Right')], colorbar=True)
     
    diff_matrix   = hv.Image(Mean_CCs[(suffix,grouping,'Awake-Drowsy','R')].values[::-1], bounds=(0,0,200,200)).opts(clim=(-0.20,0.20), cmap='PiYG_r', aspect='square', frame_width=300, 
                                                                                              title='Awake - Drowsy', fontsize={'ticks':12, 'title':18}, 
                                                                                              yticks=network_labels_tuples, xticks=[(50,'Left'),(100,'|'),(150,'Right')], colorbar=True)
    
    diff_pc_matrix = hv.Image(Mean_CCs[(suffix,grouping,'100*(Awake-Drowsy)/Awake','R')].values[::-1], bounds=(0,0,200,200)).opts(clim=(-200,200), cmap='PiYG_r', aspect='square', frame_width=300, 
                                                                                              title='Awake - Drowsy (%)', fontsize={'ticks':12, 'title':18}, 
                                                                                              yticks=network_labels_tuples, xticks=[(50,'Left'),(100,'|'),(150,'Right')], colorbar=True)
...
@pn.depends(select_preprocessing, select_grouping)
def plot_histograms(suffix,grouping):
    FC_Drowsy = Mean_CCs[(suffix,grouping,'Drowsy','R')].values
    FC_Drowsy = pd.DataFrame(FC_Drowsy[np.triu_indices(Nrois,1)], columns=['Drowsy'])
    FC_Awake  = Mean_CCs[(suffix,grouping,'Awake','R')].values
...
data_dashboard = pn.Column(pn.Row(select_preprocessing,select_grouping,inform_preprocessing),
